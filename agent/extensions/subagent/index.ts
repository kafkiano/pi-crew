...
const SubagentParams = Type.Object({
	...
	inheritContext: Type.Optional(
		Type.Union([
			Type.Boolean({ description: "Inherit recent parent session context (default: last 30 messages)" }),
			Type.Number({ description: "Number of recent messages to inherit", minimum: 1, maximum: 200 }),
		], {
			description: "Inherit conversation context from the parent session. Pass true for default (30 messages), or a number for specific count.",
		}),
	),
});

function serializeSessionContext(messages: { role: string; content: any }[], maxMessages: number): string {
	...
	for (const msg of recent) {
		...
		// Handle array content — extract text parts only (skip tool calls/results for brevity)
		if (!Array.isArray(msg.content)) continue;
		const textParts = msg.content
			.filter((p: any): p is { type: "text"; text: string } => p.type === "text" && typeof p.text === "string")
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			'Built-in agents and how to task them:',
			'',
			'- **architect** — systems architect for analysis, research, and planning. Read-only. `inheritContext: true`: pass a short task like "Review my approach to X" or "Draft an ACTION_PLAN for Y".',
			'- **coder** — implements with surgical precision. No inherited context: write a full, self-contained specification.',
			'- **tester** — runs tests, type checks, linters. No inherited context: write a full test/verification request.',
			'- **memory-recall** — retrieves relevant memories. `inheritContext: true`: pass just the query, e.g. "Find context about X".',
			'- **memory-verify** — verifies claims against stored knowledge. `inheritContext: true`: pass the claim to verify.',
			'- **memory-write** — consolidates learned knowledge. `inheritContext: true`: pass only what to consolidate, e.g. "Persist the cdp-cli skill fixes". Do not re-summarize the conversation.',
			'- **memory-admin** — knowledge graph maintenance, analytics, and bulk ops. No inherited context: describe the admin task precisely.',
			'- **visual-inspector** — takes screenshots and describes pages. `inheritContext: true`: pass the tab/page and what to look for.',
			'',
			"Use `inheritContext: true` (or a number of messages) to give a subagent the recent parent conversation. When a subagent inherits context, keep the task short and focused; the context carries the surrounding conversation. When it does not inherit context, write a complete, self-contained task.",
		...
		promptSnippet: "Delegate to specialized subagents. Use inheritContext=true for context-aware agents (architect, memory-*); write full specs for coder/tester/memory-admin.",
		promptGuidelines: [
			"When calling a subagent that has inheritContext: true, keep the task short and targeted. The parent conversation is passed automatically.",
			"When calling a subagent without inherited context (coder, tester, memory-admin), write a complete, self-contained task with all necessary context.",
			"Use parallel mode for independent subtasks and chain mode when later steps depend on earlier output via {previous}.",
			"Use action: 'list' to discover available agents if you are unsure which agent to use.",
		],
