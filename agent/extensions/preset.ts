...
/**
 * If instructions ends with .md, resolve it as a file path against getAgentDir().
 */
function resolveInstructions(instructions: string): string {
	if (!instructions.endsWith(".md")) return instructions;
	const filePath = join(getAgentDir(), instructions);
	if (existsSync(filePath)) {
		return readFileSync(filePath, "utf-8");
	}
	console.warn(`Preset instructions file not found: ${filePath}`);
	return instructions;
}
...
function loadPresets(cwd: string): PresetsConfig {
	...
	// Resolve .md file references in instructions
	for (const preset of Object.values(merged)) {
		if (preset.instructions) {
			preset.instructions = resolveInstructions(preset.instructions);
		}
	}
